...
export const sendOTP = async (email: string, otp: string, name: string) => {
    ...
    const mailOptions = {
        from: `"${process.env.SMTP_FROM_NAME || "HelPhin LMS"}" <${process.env.SMTP_FROM_EMAIL || "noreply@helphin-lms.com"}>`,
        to: email,
    ...
    try {
        ...
        console.log(`[MAILER] Email sent successfully to ${email}. MessageId: ${info.messageId}`);
        return true;
    } catch (error) {
...
export const sendSupportEmail = async (
    ...
    const mailOptions = {
        from: `"${process.env.SMTP_FROM_NAME || "HelPhin LMS"}" <${process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER}>`,
        to: companyEmail,
    ...
    console.log("[MAILER] Attempting to send support email to:", companyEmail);
    console.log("[MAILER] From:", mailOptions.from);

    try {
        const info = await transporter.sendMail(mailOptions);
        console.log(`[MAILER] Support email sent. MessageId: ${info.messageId}`);
        return true;
    } catch (error: any) {
        console.error(`[MAILER] Error sending support email:`, error?.message || error);
        console.error(`[MAILER] Error code:`, error?.code);
        console.error(`[MAILER] Response code:`, error?.responseCode);
        return false;
    }
};
