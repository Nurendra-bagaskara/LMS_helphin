...
export default function MatkulProdiLainMkDetail() {
      ...
      const fetchTopik = async (url: string, type: TopikItem["type"], prefix: string): Promise<TopikItem[]> => {
        try {
          ...
          if (d.success && Array.isArray(d.data)) {
            return d.data
              .filter((item: any) => (item.mataKuliahId === mkId || item.courseId === mkId))
              .map((item: any) => ({
                  id: `${type}-${item.id}`,
                  realId: item.id,
                  title: item.title,
  ...
  return (
    <div className={`min-h-screen bg-[#F8FBFF] ${inter.className} pb-10 flex flex-col gap-6`}>
      {/* ══ HERO SECTION ══ */}
      <section className="mx-4 mt-4 relative overflow-hidden rounded-[32px] bg-gradient-to-br from-[#0055FF] via-[#068DFF] to-[#07D1FF] shadow-xl shadow-blue-200/50 min-h-[240px] flex">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-10 py-8">
          <button
            onClick={() => router.push(`/student/matkul-prodi-lain/${prodiId}`)}
            className="flex items-center gap-2 text-white/70 hover:text-white mb-4 text-sm font-medium transition-colors w-fit"
          >
            <ArrowLeft size={16} />
      ...
      <section className="px-6 flex flex-col gap-6 max-w-[1400px] mx-auto w-full">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-black text-slate-800">Topik Pembelajaran</h2>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative min-w-[280px]">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-white border border-slate-100 rounded-2xl shadow-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all text-sm font-semibold text-slate-600"
              />
            </div>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 px-5 py-3 bg-white text-slate-700 rounded-2xl border border-slate-100 shadow-sm font-bold text-sm hover:border-blue-200 transition-all"
              >
              ...
              {showFilterMenu && (
                <>
                  ...
                  <div className="absolute top-full right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl shadow-slate-200/50 border border-slate-100 p-2 z-30">
                    {filters.map(f => (
                      <button
                        key={f.key}
            ...
            <div className="relative">
              ...
              {showYearMenu && (
                <>
                  ...
                  <div className="absolute top-full right-0 mt-3 w-48 bg-white rounded-2xl shadow-2xl shadow-slate-200/50 border border-slate-100 p-2 z-30">
                    {years.map(y => (
                      <button
                        key={y}
                        onClick={() => { setSelectedYear(y); setShowYearMenu(false); }}
                        className={`w-full flex items-center p-3 rounded-xl text-sm font-bold transition-all ${selectedYear === y ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}
                      >
                        {y}
                      </button>
                    ))}
        ...
        <div className="grid grid-cols-1 gap-4 pb-10">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border-2 border-slate-50 border-dashed">
              <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-6">
                <BookOpen size={40} className="text-slate-200" />
              </div>
              <h3 className="text-xl font-black text-slate-700 mb-2">Belum ada topik ditemukan</h3>
              <p className="text-slate-400 max-w-xs text-center font-medium">Coba gunakan filter lain atau hubungi admin untuk update materi terbarunya.</p>
            </div>
          ) : (
            filtered.map((item) => (
...
function TopikCard({ item }: { item: TopikItem }) {
  ...
  return (
    <div
      ...
      className="group relative bg-white rounded-3xl border border-slate-100 overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-xl hover:shadow-blue-200/20 hover:-translate-y-1 transition-all duration-300"
    >
      {/* Decorative Vector */}
      <img src={VECTOR21_B64} alt="" className="absolute -bottom-4 -left-4 w-32 h-auto opacity-10 group-hover:opacity-25 transition-opacity pointer-events-none" />
      ...
      <div className="p-8 flex-1 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3 mb-1">
            ...
            {item.tahunAjaran && (
              <span className="text-[10px] font-bold text-slate-300 tracking-wider">TA {item.tahunAjaran}</span>
            )}
          </div>
          ...
          {item.subtitle && (
            <p className="text-sm text-slate-500 font-medium line-clamp-1">{item.subtitle}</p>
          )}
        </div>
